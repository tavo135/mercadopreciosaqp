...
async function askAI() {
  try {
    ...
    if (!KEY) {
      alert(" KEY IS EMPTY! Check your .env file");
      return;
    }
    ...
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "user", content: "The best price for a kilo of onions in Arequipa, Peru? Answer in one short sentence." }
        ]
      })
    });
    ...
    if (data.choices?.length > 0) {
      alert("🤖 AI: " + data.choices[0].message.content);
    }
  } catch (err) {
